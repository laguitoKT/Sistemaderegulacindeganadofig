...
import { Home, Users, FileText, User, Lock, Edit3, Save, X, CheckCircle, Clock, Eye, EyeOff, AlertTriangle, Shield, FileCheck, Activity } from 'lucide-react';
...
const perfilData = {
  ...
  ultimoAcceso: '2026-03-13',
};

const cambiosPendientesMock: CambioPendiente[] = [
  {
    campo: 'Correo Electrónico',
    valorActual: 'admin@sistema-ganado.gob.mx',
...
export default function AdminPerfil() {
  ...
  const hasChanges = () => {
    return (
      ...
      editData.departamento !== perfilData.departamento ||
  ...
  const confirmSaveChanges = () => {
    ...
    setEditMode(false);
    setShowSuccessMsg('Tu solicitud de cambio ha sido enviada y está pendiente de aprobación por otro administrador.');
  ...
  const getChangedFields = () => {
    ...
    if (editData.nombre !== perfilData.nombre) changes.push({ campo: 'Nombre', anterior: perfilData.nombre, nuevo: editData.nombre });
    ...
    if (editData.departamento !== perfilData.departamento) changes.push({ campo: 'Departamento', anterior: perfilData.departamento, nuevo: editData.departamento });
    if (editData.municipio !== perfilData.municipio) changes.push({ campo: 'Municipio', anterior: perfilData.municipio, nuevo: editData.municipio });
    if (editData.estado !== perfilData.estado) changes.push({ campo: 'Estado', anterior: perfilData.estado, nuevo: editData.estado });
    return changes;
  };
      ...
      <main className="flex-1 p-10 pb-[140px]">
        ...
        {showSuccessMsg && (
          <div className="mb-6 bg-[#f0e8e3] border border-[#d4c4b8] rounded-xl p-4 flex items-center gap-3">
            <CheckCircle className="w-5 h-5 text-[#5B321A] flex-shrink-0" />
            <p className="text-[#5B321A]">{showSuccessMsg}</p>
          </div>
        )}
        ...
        <div className="mb-8">
          <div>
            ...
            <p className="text-lg text-gray-600">Consulta y gestiona tu información de administrador</p>
          </div>
        </div>
        ...
        {editMode && (
          <div className="mb-6 bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-amber-700 text-sm mt-1">Incluso como administrador, las modificaciones a tu información personal deben ser aprobadas por otro administrador para mantener la integridad del sistema. Solo el cambio de contraseña es inmediato.</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow p-6">
            ...
            <p className="text-xl font-bold text-gray-900">
              {new Date(perfilData.fechaRegistro).toLocaleDateString('es-MX', { year: 'numeric', month: 'long' })}
            </p>
          </div>
          <div className="bg-white rounded-xl shadow p-6">
            <p className="text-sm text-gray-500 mb-1">Último Acceso</p>
            <p className="text-xl font-bold text-gray-900">
              {new Date(perfilData.ultimoAcceso).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' })}
            </p>
          </div>
        </div>
              ...
              <div>
                ...
                <p className="text-lg text-gray-900">
                  <span className="inline-flex items-center gap-2">
                    {perfilData.rol}
                    <span className="px-2 py-0.5 bg-[#f0e8e3] text-[#5B321A] text-xs rounded-full font-medium">Activo</span>
                  </span>
                </p>
              </div>
              <div>
                ...
                {editMode ? (
                  ...
                  <p className="text-lg text-gray-900">{perfilData.departamento}</p>
                )}
              </div>
              <div>
        ...
        <div className="bg-white rounded-xl shadow p-6 flex gap-3 justify-center mb-8">
          ...
          {!editMode ? (
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-3 bg-[#5B321A] hover:bg-[#4a2817] text-white rounded-lg font-medium transition-colors"
            >
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={!hasChanges()}
                className="inline-flex items-center gap-2 px-5 py-3 bg-[#5B321A] hover:bg-[#4a2817] text-white rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <div className="p-6 space-y-5">
              <div className="bg-[#f0e8e3] border border-[#d4c4b8] rounded-lg p-3">
                <p className="text-sm text-[#5B321A] flex items-center gap-2">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 pr-12 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#5B321A] focus:border-transparent"
            ...
            <div className="p-6 border-t border-gray-200 flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-3 bg-[#5B321A] hover:bg-[#4a2815] text-white rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="p-6 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-500 mt-1">Los siguientes cambios serán enviados para aprobación por otro administrador</p>
            </div>
              ...
              {getChangedFields().map((change, idx) => (
                <div key={idx} className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-sm text-gray-500">
                    ...
                    <span className="text-[#5B321A] font-medium">{change.nuevo}</span>
                  </p>
                </div>
              ))}
            </div>
            <div className="p-6 border-t border-gray-200 flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-3 bg-[#5B321A] hover:bg-[#4a2815] text-white rounded-lg font-medium transition-colors"
              >
                      ...
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-500">
                            ...
                            <span className="mx-2">→</span>
                            <span className="text-[#5B321A] font-medium">{cambio.valorNuevo}</span>
                          </p>
                        </div>
                      </div>
